...
public class SeamCarver {
    ...
    private int pwidth;
    private int pheight;
    private int nsv;  // number of vertical seams
    private int nsh;  // number of horizontal seams
    private int pdim;
    private double[] ener;
    private int[] edgeTo;
...
//        printAdj(false);
    ...
    private double ener(int index, boolean vertical)
    {
        ...
        if (vertical) 
            ...
            index1 = index / (pheight - nsh) + (index % (pheight - nsh)) * (pwidth - nsv);
        ...
        if (nsv == 0) 
            ...
            for (int j = 0; j < pdim(); j++) {
                if (ener[j] == Double.NEGATIVE_INFINITY) continue;
                cnt++;
                if (cnt == index1) {
                    result = ener[cnt];
                    break;
    ...
    private int pdim() {
        return (pheight - nsh) * (pwidth - nsv);
    }

    private int pheight(boolean vertical) {
        if (vertical) return (pheight - nsh);
        return (pwidth - nsv);
    }

    private int pwidth(boolean vertical) {
        if (vertical) return (pwidth - nsv);
        return (pheight - nsh);
    }
    ...
    private int[] findSeam(boolean v)
    {
        ...
        StdOut.printf("pwidth=%d pheight=%d\n", pwidth(v), pheight(v));
...
//        printEner(v);
    ...
    public void removeHorizontalSeam(int[] a) {
        int ind;
        for (int x = 0; x < pwidth(false); x++) {
            ind = x * pheight(false); 
            for (int y = 0; y < pheight(false); y++) { 
                if (ener[ind] == Double.NEGATIVE_INFINITY) continue;
                if (ind == a[x]) ener[ind] = Double.NEGATIVE_INFINITY; 
                ind++;
            }
        }
        nsh++;     
    ...
    public void removeVerticalSeam(int[] a) {
        int ind;
        for (int x = 0; x < pheight(true); x++) {
            ind = x * pwidth(true);
            for (int y = 0; y < pwidth(true); y++) { 
                if (ener[ind] == Double.NEGATIVE_INFINITY) continue;
                if (ind == a[x]) ener[ind] = Double.NEGATIVE_INFINITY; 
                ind++;
            }
