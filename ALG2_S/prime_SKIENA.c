...
prim(graph *g, int start)           /* PRIM MINIMUM SPANNING TREE */
{
    ...
    while (intree[v] == FALSE) {
		printf ("=========================================================================\n",v);
		printf("Min spanning tree v=%d\n",v);
        intree[v] = TRUE;
        p = g->edges[v];
        while (p != NULL) {
			w = p->y;
			weight = p->weight;
			printf("distance[%d]=%d weight=%d\n", w,distance[w], weight);
			if ((distance[w] > weight) && (intree[w] == FALSE)) {
				distance[w] = weight;
				parent[w] = v;
			    printf("--- distance[%d]=%d parent[%d]=%d\n", w,distance[w], w,parent[w]);
			}
			p = p->next;
		}
		...
		dist = MAXINT;
		for (i=1; i<=g->nvertices; i++) {
			printf("--- intree[%d]=%d dist=%d distance[%d]=%d\n", i,intree[i], dist, i,distance[i]);
