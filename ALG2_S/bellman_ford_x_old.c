...
int bellman_ford(graph * g, int src)
{
    int V = g->n;
    int E = g->m;
    int i, j, u, v, u1, v1, weight, nwc, wienner;
	vertex *p;
	printf("V=%d E=%d\n", V, E);
        ...
        for (j = 0; j < V; j++) {
            p = (vertex *) g->v[j]->next;
  		    while (p != NULL) {
                ...
                v = p->vid;  v1 = v - 1;
                weight = p->ew;
                if (g->dist[u1] + weight < g->dist[v1])  /* vertex numbers are from 1..n    */
    ...
    for (j = 0; j < V; j++) {
		if (nwc == TRUE) break;
		if (g->dist[j] < wienner) wienner = g->dist[j];
         p = (vertex *) g->v[j]->next;
 	    while (p != NULL) {
            u = g->v[j]->vid; u1 = u - 1;
            v = p->vid; v1 = v - 1;
...
{
    int mode = 1, debug = FALSE, wienner, res, i;
    graph g;
