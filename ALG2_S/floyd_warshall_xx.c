...
*/
	...
	for (i=1; i<=m; i++) {
	     scanf("%d %d %d",&x,&y,&w);
/*	     g->weight[x][y][0] = w;  */
	     g->weight[x][y][0] = 1;   /*  CHANGE !!!! */
	     if (directed == FALSE) g->weight[y][x][0] = w;
    }
	printf("========== loop 2 done =============\n");
/*
	for (i=1; i<=g->nvertices; i++) {
...
print_adjm1(adjacency_matrix *g, int k)
{
	int i,j;            /* counter */
...
floyd(adjacency_matrix *g)
{
	...
	for (k=1; k<=g->nvertices; k++)
	    for (i=1; i<=g->nvertices; i++)
	          for (j=1; j<=g->nvertices; j++) {
				  g->weight[i][j][k] = g->weight[i][j][k-1] + g->weight[i][k][k-1] * g->weight[k][j][k-1];
/*
...
main(int argc, char *argv[])
{
	...
	floyd(&g);
	printf("========== print graph =============\n");
	print_adjm(&g,g.nvertices);
	print_adjm1(&g,g.nvertices);
/*
	printf("========== min distance =============\n");
